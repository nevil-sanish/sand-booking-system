...
export const ParticleCard = ({
  ...
  useEffect(() => {
    ...
    const handleMouseEnter = () => {
      isHoveredRef.current = true; animateParticles();
    ...
    return () => {
      isHoveredRef.current = false; element.removeEventListener('mouseenter', handleMouseEnter); element.removeEventListener('mouseleave', handleMouseLeave); element.removeEventListener('mousemove', handleMouseMove); element.removeEventListener('click', handleClick); clearAllParticles();
    };
...
export const GlobalSpotlight = ({ gridRef, disableAnimations = false, enabled = true, spotlightRadius = DEFAULT_SPOTLIGHT_RADIUS, glowColor = DEFAULT_GLOW_COLOR }) => {
  ...
  useEffect(() => {
    ...
    spotlight.style.cssText = `position: fixed; width: 800px; height: 800px; border-radius: 50%; pointer-events: none; background: radial-gradient(circle, rgba(${glowColor}, 0.15) 0%, rgba(${glowColor}, 0.08) 15%, transparent 70%); z-index: 200; opacity: 0; transform: translate(-50%, -50%); mix-blend-mode: screen;`;
    ...
    const handleMouseMove = e => {
      ...
      const cards = gridRef.current.querySelectorAll('.magic-bento-card, .particle-card, .card');

      if (!mouseInside) {
        gsap.to(spotlightRef.current, { opacity: 0, duration: 0.3, ease: 'power2.out' });
        cards.forEach(card => card.style.setProperty('--glow-intensity', '0')); return;
      }
      ...
      cards.forEach(card => {
        ...
        const dist = Math.hypot(e.clientX - (cardRect.left + cardRect.width / 2), e.clientY - (cardRect.top + cardRect.height / 2)) - Math.max(cardRect.width, cardRect.height) / 2;
        minDistance = Math.min(minDistance, Math.max(0, dist));
      ...
      gsap.to(spotlightRef.current, { opacity: minDistance <= proximity ? 0.8 : minDistance <= fadeDistance ? ((fadeDistance - minDistance) / (fadeDistance - proximity)) * 0.8 : 0, duration: 0.2, ease: 'power2.out' });
    };

    const handleMouseLeave = () => {
      gridRef.current?.querySelectorAll('.magic-bento-card, .particle-card, .card').forEach(card => card.style.setProperty('--glow-intensity', '0'));
